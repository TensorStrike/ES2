...
class BasicBlock_NoPara(nn.Module):
    ...
    def forward(self, x, weight_params):
        out = F.conv2d(x, weight_params[0], stride=self.stride, padding=1)
        out = self.bn1(out)
        out = F.relu(out)
        out = F.conv2d(out, weight_params[1], stride=1, padding=1)
        out = self.bn2(out)
        out += self.shortcut(x)
        out = F.relu(out)
        return out
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self._forward_impl(self.layer1, out)
        out = self._forward_impl(self.layer2, out)
        out = self._forward_impl(self.layer3, out)
        out = self._forward_impl(self.layer4, out)
        out = F.avg_pool2d(out, 4)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        return out

    def _forward_impl(self, layers, x):
        for i, layer in enumerate(layers):
            if i < self.ratio:
                x = layer(x)
            else:
                weight_params = [layers[self.ratio-1].conv1.weight, layers[self.ratio-1].conv2.weight]
                x = layer(x, weight_params)
        return x
    ...
    def get_shared_para(self):
        ...
        for layer in [self.layer1, self.layer2, self.layer3, self.layer4]:
            if len(layer) > self.ratio:
                ...
                shared_para += shared_block.conv1.weight.numel() + shared_block.conv2.weight.numel()
        return shared_para
