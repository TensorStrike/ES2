...
class BasicBlock(nn.Module):    # for weight sharing
    expansion = 1
    ...
    def get_params(self):
        # get parameters for weight sharing
        return [self.conv1.weight, self.conv2.weight, self.shortcut[0].weight if len(self.shortcut) > 0 else None]
...
class BasicBlock_NoPara(nn.Module):
    ...
    def forward(self, x, shared_params):
        conv1_weight, conv2_weight, shortcut_weight = shared_params

        out = self.relu1(self.bn1(F.conv2d(x, conv1_weight, stride=self.stride, padding=1)))
        out = self.bn2(F.conv2d(out, conv2_weight, stride=1, padding=1))

        if shortcut_weight is not None:
            shortcut = F.conv2d(x, shortcut_weight, stride=self.stride)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        out = self.relu(self.bn1(self.conv1(x)))
        out = self._forward_layer(self.layer1, out)
        out = self._forward_layer(self.layer2, out)
        out = self._forward_layer(self.layer3, out)
        out = self._forward_layer(self.layer4, out)
