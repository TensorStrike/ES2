...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.relu2(out)
        if torch.isnan(out).any():
            print("NaN detected in BasicBlock after shortcut and final relu")
        return out

    def getParams(self):
        weights_params = []
        weights_params.append(self.conv1.weight)
        weights_params.append(self.conv2.weight)
        if len(self.shortcut) > 0:
            weights_params.append(self.shortcut[0].weight)
        return weights_params, None
...
class BasicBlock_NoPara(nn.Module):
    ...
    def forward(self, x, weight_params, bias_params=None):
        ...
        if len(weight_params) == 2:
            ...
            out += self.shortcut(F.conv2d(x, weight_params[2], bias=None, stride=self.stride))
        out = self.relu2(out)
        if torch.isnan(out).any():
            print("NaN detected in BasicBlock_NoPara after shortcut and final relu")
        return out
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, ratio=2):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu1 = DyReLUB(64, reduction=4, k=2, conv_type='2d')
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
